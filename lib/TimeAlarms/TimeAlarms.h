...
// or not allocated
#define dtIsAlarm(_type_)                                                      \
  (_type_ >= dtExplicitAlarm && _type_ < dtLastAlarmType)
...
private:
  ...
  AlarmID_t create(time_t value, OnTick_t onTickHandler, uint8_t isOneShot,
                   dtAlarmPeriod_t alarmType, uint8_t isEnabled = true);
...
public:
  ...
  AlarmID_t
  triggerOnce(time_t value,
              OnTick_t onTickHandler); // trigger once at the given time_t

  AlarmID_t
  alarmRepeat(time_t value,
              OnTick_t onTickHandler); // trigger daily at given time of day
  AlarmID_t alarmRepeat(const int H, const int M, const int S,
                        OnTick_t onTickHandler); // as above, with hms arguments
  AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, const int H, const int M,
                        const int S,
                        OnTick_t onTickHandler); // as above, with day of week

  AlarmID_t
  alarmOnce(time_t value,
            OnTick_t onTickHandler); // trigger once at given time of day
  AlarmID_t alarmOnce(const int H, const int M, const int S,
                      OnTick_t onTickHandler); // as above, with hms arguments
  ...
  AlarmID_t timerOnce(
      time_t value,
      OnTick_t onTickHandler); // trigger once after the given number of seconds
  AlarmID_t timerOnce(const int H, const int M, const int S,
                      OnTick_t onTickHandler); // As above with HMS arguments

  AlarmID_t timerRepeat(time_t value, OnTick_t onTickHandler); // trigger after
                                                               // the given
                                                               // number of
                                                               // seconds
                                                               // continuously
  AlarmID_t timerRepeat(const int H, const int M, const int S,
                        OnTick_t onTickHandler); // As above with HMS arguments

  void delay(unsigned long ms);

  // utility methods
  uint8_t getDigitsNow(dtUnits_t Units); // returns the current digit value for
                                         // the given time unit
  ...
  dtAlarmPeriod_t
  readType(AlarmID_t ID); // return the alarm type for the given alarm ID
...
private: // the following methods are for testing and are not documented as part
  ...
  bool isAlarm(AlarmID_t ID); // returns true if id is for a time based alarm,
...
#define AlarmHMS(_hr_, _min_, _sec_)                                           \
  (_hr_ * SECS_PER_HOUR + _min_ * SECS_PER_MIN + _sec_)
